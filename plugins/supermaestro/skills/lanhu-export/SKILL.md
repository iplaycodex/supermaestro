...
- 中断后使用同一 `--url`、`--group`、`--out` 并增加 `--resume`。每个画板完成后原子保存 manifest；只有所有画板完成才标记 `complete: true`。
- 每次仍查询最新版本，按 `image_id + version_id` 和本地 SHA-256 验证缓存；文件缺失、损坏、版本变化或旧 manifest 缺少哈希时重新下载。schema 已成功而图片失败时，可单独复用 schema。
- `--concurrency 1` 默认串行，允许显式设置 2–4；输出顺序不随完成顺序改变。
- 临时网络错误和 408/429/500/502/503/504 最多尝试三次；鉴权、重定向和数据格式错误立即失败。失败日志继续脱敏。
- 缓存路径越界、symlink 或项目/分组身份不匹配时拒绝复用。不要通过清空错误、移动缓存或伪造版本号规避检查。
- 新文件名含画板/版本摘要；使用 manifest 路径引用产物，不依赖文件名推断画板。
