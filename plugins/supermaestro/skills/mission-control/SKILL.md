...
- Gate 1 可用 `superpowers:brainstorming` 生成集中问题清单；用户回答后必须回填主工作台文档，问题清单只保留审阅记录。
...
- Superpowers 作为执行方法栈接入，但不替代 Mission Control 主控：本技能继续负责 workbench、Gate、状态、review pack、worktree 和集成；Gate 1 需求澄清可吸收 `superpowers:brainstorming`；计划粒度优先吸收 `superpowers:writing-plans` 的“文件/步骤/测试/命令/预期结果”格式；真实多 agent 执行优先使用 `superpowers:subagent-driven-development`；不开 subagent 或跨会话串行执行时才使用 `superpowers:executing-plans`。
...
- Superpowers 不是可选文案。凡本技能声明“必须/优先/默认吸收”的场景，必须先实际读取或调用对应 `superpowers:*` skill，并在 `reports/validation.md` 的“Superpowers 调用证据”表记录 `已读取/已调用/已吸收/已执行` 等证据；只在 `task-plan.md` 写“计划使用”不算通过。脚本硬闸门会在以下动作拒绝缺证据流程：
  - `check-workbench` / `approve-gate1`：必须有 `specs/requirement-alignment.md` 且记录用户已确认的需求对齐证据；若存在 `specs/gate-1-brainstorming-questions.md`，还必须答完并完成主文档 fan-in。
...
```bash
node <plugin-root>/scripts/supermaestro.js check <需求工作台> --action code --ui true --boards "<画板名>" --schemas "../source/ui/schemas/<file>.json" --schema-extract "specs/ui-schema-extract.md" --baselines "../source/ui/images/<file>.png"
```

图片已删除或不提供基线图时，显式使用 schema-only：

```bash
node <plugin-root>/scripts/supermaestro.js check <需求工作台> --action code --ui true --schema-only true --boards "<画板名>" --schemas "../source/ui/schemas/<file>.json" --schema-extract "specs/ui-schema-extract.md"
```
...
└── workbench/
    ...
    │   ├── gate-1-brainstorming-questions.md # 可选；如存在，Gate 1 前必须答完并 fan-in
    │   ├── api-spec.md
...
```bash
node <plugin-root>/scripts/supermaestro.js init <需求工作台> --name "<需求名>"
```
...
```bash
node <plugin-root>/scripts/supermaestro.js status <需求工作台>
```

查看推荐下一步：

```bash
node <plugin-root>/scripts/supermaestro.js next <需求工作台>
```

恢复长流程上下文：

```bash
node <plugin-root>/scripts/supermaestro.js resume <需求工作台>
```
...
```bash
node <plugin-root>/scripts/supermaestro.js check-workbench <需求工作台>
```
...
```bash
node <plugin-root>/scripts/supermaestro.js approve-gate1 <需求工作台> --confirmed-by user --confirmation "<用户确认原话或摘要>"
```

Gate 2 计划确认：

```bash
node <plugin-root>/scripts/supermaestro.js approve-gate2 <需求工作台> --mode <main-serial|single-worktree-serial|multi-worktree-parallel> --confirmed-by user --confirmation "<用户确认原话或摘要>" --worktree <true|false> --subagents <true|false> --checkpoint <true|false>
```

Gate 3 Review 请求和确认：

```bash
node <plugin-root>/scripts/supermaestro.js request-gate3 <需求工作台> --review-pack reviews/review-packs.md --validation reports/validation.md
node <plugin-root>/scripts/supermaestro.js approve-gate3 <需求工作台> --review true --validation true
```

Gate 4 最终动作请求和确认：

```bash
node <plugin-root>/scripts/supermaestro.js request-gate4 <需求工作台>
node <plugin-root>/scripts/supermaestro.js approve-gate4 <需求工作台> --merge false --commit false --push false --cleanup false
```
...
```bash
node <plugin-root>/scripts/supermaestro.js verify <需求工作台> --strict true
```
...
- Gate 1 前如果还有范围、规则、API/UI 或验收口径模糊点，读取并使用 `superpowers:brainstorming`，生成 `specs/gate-1-brainstorming-questions.md`；用户回答后必须把答案 fan-in 到 `context.md`、`specs/requirement-alignment.md`、`plans/progress.md`，以及适用时的 `specs/page-contract-matrix.md`。
...
如果使用了 `superpowers:brainstorming`，必须在批准 Gate 1 前完成“问题 -> 用户答案 -> 主文档回填”的 fan-in：

| 问题来源 | 用户答案位置 | 回填目标 | 状态 |
| --- | --- | --- | --- |
| `specs/gate-1-brainstorming-questions.md` | `你的答案` 区块 | `context.md` / `specs/requirement-alignment.md` / `plans/progress.md` / `specs/page-contract-matrix.md` | pending / synced |
...
如果检查失败，先补齐缺失文档再进入 Gate 1。`approve-gate1` 也会执行同样检查，缺少 `context.md`、`specs/requirement-alignment.md`、必要的 `specs/api-spec.md` / UI 规格 / 页面契约矩阵、未回答或未 fan-in 的 Gate 1 brainstorming 问题，或需求对齐文档未记录用户已确认摘要时不得确认。
