...
- 识别用户已有改动，避免触碰或回滚。
- 选择稳定的基准分支。
- 派发前先定义唯一的 `target`、`branch`、`base` 三元组；默认使用主仓库同级目录 `<repo>.worktrees/<task-id>`。
- target 必须位于 `state.sourceRoot` 外，不能使用项目内 `.worktrees/` /
  `worktrees/`，也不能位于系统临时目录。
...
不得使用 `/tmp`、`/private/tmp`、系统临时目录或其他易清理位置创建
worktree。如果项目旁目录不可用，必须先说明阻塞并调整持久目录方案。

## 创建与所有权登记

每个 worktree 都必须完成以下三步，且全程使用 Plan 中同一组
`target/branch/base`：
...
第一步只记录本次精确创建意图。第二步由调用方执行真实 Git 操作。第三步由
CLI 对照 `git worktree list`，校验目标路径、branch 和 base，成功后才写入
`state.json` 的 owned worktree registry。

`--target` 可传绝对路径，或相对 `state.sourceRoot` 的路径；相对路径固定
按该源码根解析，不受当前 cwd 影响。CLI 在意图和 registry 中统一保存
canonical absolute 路径。

- `ALLOW` 不表示 worktree 已创建，`git worktree add` 成功也不表示已登记。
- 登记失败时停止；不得派发 worker、同步物料、创建 checkpoint commit，
  也不得把目标声明为本流程拥有。
- 已存在但没有走完本流程的外部 worktree 不自动收编。
- CLI 只校验、登记和授权，不执行 `git worktree add/remove`，也不创建或
  删除分支。

路径应整体加引号。命令优先保持单行；需要续行时，Bash 使用 `\`、
...
- worktree 模式执行 checkpoint commit 前，运行
  `check <workbench> --action checkpoint-commit --target "<path>"`；目标未登记
  或已漂移时不得继续。
...
收尾动作受 SuperMaestro Final Gate 控制，并遵循先验证、再选择交付动作、最后按授权清理的顺序。
...
- 给用户的 Final Brief 必须明确动作组合：本地 merge、push/PR、keep as-is、discard、cleanup worktree、delete branch。
...
- cleanup 必须先用
  `approve-final <workbench> ... --cleanup true --target "<path>"` 授权，再用
  同一目标运行 `check <workbench> --action cleanup --target "<path>"`。只有
  owned registry 中仍存在，且当前 Git 路径、branch、base 与登记信息一致
  的目标才能 `ALLOW`。
- 获得 `ALLOW` 后仍由调用方执行
  `git -C "<state.sourceRoot>" worktree remove "<path>"`；CLI 不自动删除
  目录或分支。
- `worktrees/plan.md` 只是人工投影，不能代替 registry。未登记、external、
  detached、已漂移、不存在或来源不明的工作区一律不清理。
