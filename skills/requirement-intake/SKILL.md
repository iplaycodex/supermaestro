...
description: 当用户接到一个新的软件需求，希望 Codex 收集 PRD、接口文档、蓝湖/UI、mock、截图或其他研发物料，生成 documents/<需求名>/source + workbench 工作台，按需调用 export-lanhu-version 和 prd-structure，并初始化 mission-control 规划、停在 Gate 1 等待确认时使用。
---
...
- 遇到蓝湖 stage 链接、版本分组或 UI schema 导出需求时，使用 `$export-lanhu-version`。
...
5. 如果用户提供蓝湖 stage 链接或分组名，调用 `$export-lanhu-version`，默认以 schema-only 方式导出到 `source/ui/`；只有用户明确要求视觉基线时才导出图片。
6. 如果存在 PRD 物料，调用 `$prd-structure`，默认把结构化结果放入 `workbench/specs/`，除非用户指定其他路径。
7. 调用 `$mission-control` 初始化或刷新工作台，完成 UI 体检、物料索引、共享上下文、任务计划、进度表、Review Pack 骨架和验证报告。
8. 输出简短的 Gate 1 Decision Brief，说明范围、缺失物料、UI/API 健康状态、待确认问题、推荐执行档位、审查成本和需要用户确认的具体决策。
